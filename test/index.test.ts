...
describe('listData function', () => {
  ...
  it('encodeResponse', async () => {
    const response: AttNetworkResponseResolve[] = createNetworkResponseResolve();
  ...
  it('supports Solana Program Derived Addresses (PDAs)', async () => {
    const { PublicKey } = await import('@solana/web3.js');
    const { isSolanaAddress } = await import('../src/utils');
    const programId = new PublicKey('11111111111111111111111111111111');
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from('test')], programId);
    const pdaAddress = pda.toBase58();

    expect(isSolanaAddress(pdaAddress)).toBe(true);
    expect(isSolanaAddress('0x7ab44DE0156925fe0c24482a2cDe48C465e47573')).toBe(false);
    expect(isSolanaAddress('invalid-solana-address-that-is-not-base58')).toBe(false);

    const att = createSolanaAttestation();
    att.recipient = pdaAddress;
    const encoded = encodeAttestation(att);
  ...
  function createNetworkResponseResolve() {
    const response: AttNetworkResponseResolve[] = new Array<AttNetworkResponseResolve>(3);
    for (let i = 0; i < 3; i++) {
      response[i] = {
